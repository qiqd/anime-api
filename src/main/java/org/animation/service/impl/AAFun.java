...
public class AAFun implements HtmlParser, Serializable {
  final static String NAME = "风铃动漫";
  final static String LOGOURL = "https://p.upyun.com/demo/tmp/Hds66ovM.png";
  ...
  public List<Animation> fetchSearchSync(String keyword, Integer page, Integer size) throws Exception {
    String searchUrl = "/feng-s.html?wd=" + keyword;
    ...
    return li.stream().map(item -> {
      ...
      animation.setCoverUrls(List.of(a.attr("data-original")));
  ...
  public AnimationDetail fetchDetailSync(String videoId) throws Exception {
    Element document = Jsoup.connect(BASEURL + videoId).get().body();
    ...
    if (!picElements.isEmpty()) {
      ...
      animation.setCoverUrls(List.of(Optional.of(picElement.attr("data-original")).orElse("")));
    }
...
// 解析评分
  ...
  public PlayInfo fetchPlayInfoSync(String episodeId) throws Exception {
    ...
    Element varPlayerAaaa = script.stream().filter(item -> item.data().contains("var player_aaaa")).toList().get(0);
    ...
    String decodeUrl = URLDecoder.decode(playerData.getUrl(), StandardCharsets.UTF_8);
